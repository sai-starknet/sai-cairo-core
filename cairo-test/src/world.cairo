...
use sai::world::{world, IWorldDispatcher, IWorldDispatcherTrait, WorldStorageTrait, WorldStorage};
        ...
        for c in contracts {
            let contract_address = match c.contract {
                ...
                ContractDescriptor::Named((
                    ...
                    let selector = sai::utils::selector_from_names(*namespace, *name);
                    match (*self.dispatcher).resource(selector) {
                        sai::world::Resource::Contract((address, _)) => address,
