...
use sai::meta::Layout;
use sai::model::{Model, ModelIndex, ModelStore};
use sai::storage::{database, storage};
use sai::world::{IWorldDispatcher, IWorldDispatcherTrait};
...
fn test_struct_with_many_fields_fixed() {
    ...
    let layout = match sai::model::Model::<PositionWithQuaterions>::layout() {
...
fn bench_nested_struct_packed() {
    ...
    let layout = match sai::model::Model::<Case>::layout() {
        Layout::Fixed(layout) => layout,
        _ => panic!("expected fixed layout"),
    };
...
fn bench_complex_struct_packed() {
    ...
    let layout = match sai::model::Model::<Character>::layout() {
        Layout::Fixed(layout) => layout,
        _ => panic!("expected fixed layout"),
    };
