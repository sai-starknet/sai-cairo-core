...
use sai::world::{world, IWorldDispatcherTrait};
use sai::contract::components::upgradeable::{IUpgradeableDispatcher, IUpgradeableDispatcherTrait};
use sai::meta::{IDeployedResourceDispatcher, IDeployedResourceDispatcherTrait};
...
pub mod test_contract_upgrade {
    ...
    use sai::contract::components::world_provider::IWorldProvider;
    ...
    #[abi(embed_v0)]
    pub impl ContractImpl of sai::meta::interface::IDeployedResource<ContractState> {
...
fn test_upgrade_contract_from_resource_owner() {
    ...
    if let world::Event::ContractUpgraded(event) = event.unwrap() {
        assert(
            event
                .selector == sai::utils::selector_from_namespace_and_name(DOJO_NSH, @contract_name),
...
fn test_upgrade_contract_from_resource_writer() {
    ...
    let contract = IDeployedResourceDispatcher { contract_address };
    let contract_name = contract.dojo_name();
    let contract_selector = sai::utils::selector_from_namespace_and_name(DOJO_NSH, @contract_name);
