use sai::world::Resource;
use sai::world::world::Event as WorldEvent;
use sai::utils::bytearray_hash;
use sai::world::{
...
use sai::event::{Event, EventStorage};
...
fn test_can_call_init_only_world() {
    ...
    match starknet::syscalls::call_contract_syscall(
        address, sai::world::world::DOJO_INIT_SELECTOR, [].span()
    ) {
...
fn test_can_call_init_only_world_args() {
    ...
    match starknet::syscalls::call_contract_syscall(
        address, sai::world::world::DOJO_INIT_SELECTOR, [123].span()
    ) {
        Result::Ok(_) => panic!("should panic"),
        Result::Err(e) => {
            let mut s = e.span();
